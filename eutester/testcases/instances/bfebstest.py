...
class BFEBSBasics(InstanceBasics):
    ...
    def clean_method(self):
        ...
        if self.volume:
            self.tester.ec2.delete_volume(self.volume)
        if self.volume_1:
            self.tester.ec2.delete_volume(self.volume_1)
        if self.volume_2:
            self.tester.ec2.delete_volume(self.volume_2)
    ...
    def RegisterImage(self):
        ...
        for instance in self.reservation.instances:
            self.volume_1 = self.tester.ec2.create_volume(zone=self.zone, size=3)
            ...
            snapshot = self.tester.ec2.create_snapshot(self.volume_1.id)
    ...
    def DetachAttachRoot(self, zone = None):
        ...
        for instance in instances:
            ...
            self.tester.ec2.attach_volume(instance, root_volume, self.root_volume_path)
    ...
    def StartTerminate(self, zone = None):
        ...
        for instance in instances:
            self.assertTrue(self.tester.ec2.start_instances(instances))
    ...
    def CreateImage(self,zone=None):
        ...
        for instance in self.reservation.instances:
            ...
            rebooted_image = self.tester.ec2.create_image(instance, "BFEBS-test-create-image-reboot-" + current_time)
            ...
            new_image_reservation = self.tester.ec2.run_image(**self.run_instance_params)
            ...
            self.tester.ec2.terminate_instances(new_image_reservation)
        ...
        for instance in self.reservation.instances:
            ...
            instance.sys("touch " + temp_file + " && sync", code=0)
            instance.sys('ls -la', code=0)
            not_rebooted_image = self.tester.ec2.create_image(instance, "BFEBS-test-create-image-noreboot-" + current_time, no_reboot=True)
            ending_uptime = instance.get_uptime()
            if ending_uptime < starting_uptime:
    ...
    def detachVolumeByPath(self, instance, path):
        ...
        vol_to_detach = self.tester.ec2.get_volumes(volume_id=vol_id)[0]
        self.tester.ec2.detach_volume(vol_to_detach)
